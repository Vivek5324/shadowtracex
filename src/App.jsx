  ...
  return {
    brute_force: {
      ...
      steps: [
        ...
        { phase: 0, delay: 300, severity: 'info', structured: true,
          sourceIp: attackerIp, sourcePort: port(), targetIp: targetIp, targetPort: 443,
    ...
    port_scan: {
      ...
      steps: [
        ...
        { phase: 0, delay: 300, severity: 'info', structured: true,
          sourceIp: attackerIp, sourcePort: port(), targetIp: targetIp, targetPort: 0,
        ...
        { phase: 0, delay: 600, severity: 'info', structured: true,
          sourceIp: attackerIp, sourcePort: port(), targetIp: targetIp, targetPort: 80,
        ...
        { phase: 1, delay: 1200, severity: 'warning', structured: true,
          sourceIp: attackerIp, sourcePort: port(), targetIp: targetIp, targetPort: 22,
        ...
        { phase: 1, delay: 2100, severity: 'warning', structured: true,
          sourceIp: attackerIp, sourcePort: port(), targetIp: targetIp, targetPort: 3306,
    ...
    apt_attack: {
      ...
      steps: [
        ...
        { phase: 0, delay: 1200, severity: 'warning', structured: true,
          sourceIp: targetIp, sourcePort: port(), targetIp: attackerIp, targetPort: 8443,
        ...
        { phase: 1, delay: 2800, severity: 'critical', structured: true,
          sourceIp: attackerIp, sourcePort: port(), targetIp: targetIp, targetPort: 445,
        ...
        { phase: 2, delay: 4800, severity: 'critical', structured: true,
          sourceIp: attackerIp, sourcePort: port(), targetIp: targetIp, targetPort: 389,
        ...
        { phase: 3, delay: 6500, severity: 'critical', structured: true,
          sourceIp: attackerIp, sourcePort: 53, targetIp: targetIp, targetPort: 53,
    ...
    botnet: {
      ...
      steps: [
        ...
        { phase: 0, delay: 300, severity: 'info', structured: true,
          sourceIp: randomIp(), sourcePort: port(), targetIp: targetIp, targetPort: 443,
...
function App() {
  ...
  const [pendingActions, setPendingActions] = useState([]); // Array of pending response actions
  ...
  const handleAuthorize = useCallback((id, text) => {
    ...
    // Lower risk
    setRiskPercentage(prev => Math.max(0, prev - 10));
    
    // Check if that was the last pending action
    setPendingActions(prev => {
      if (prev.length === 0 && !isAttacking) {
        setTimeout(() => {
          addLog({
            message: `━━━ THREAT NEUTRALIZED ━━━ DEFENSE COMPLETE`,
            severity: 'response',
  ...
  const handleTriggerAttack = useCallback((attackId, customTargetIp = '10.0.1.5') => {
    ...
    // Queue countermeasure authorizations at the end instead of auto-firing
    setTimeout(() => {
    ...
    // End attack simulation phase (but wait for manual response auth to say COMPLETE)
    setTimeout(() => {
